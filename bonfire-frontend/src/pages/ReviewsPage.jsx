...
function ReviewsPage() {
  ...
  return (
    <div className="animate-fadeIn">
      ...
      <div className="reviews-summary" style={{ marginBottom: '2rem', display: 'flex', gap: '4rem', alignItems: 'center', padding: '2rem 3rem' }}>
        {/* Pie Chart */}
        <div style={{ position: 'relative', width: '120px', height: '120px', flexShrink: 0, borderRadius: '50%', boxShadow: '0 4px 15px rgba(0,0,0,0.5)', overflow: 'hidden' }}>
          <svg width="120" height="120" viewBox="0 0 120 120" style={{ transform: 'rotate(-90deg)' }}>
            {/* Background pie slice (Negative) */}
            <circle cx="60" cy="60" r="60" fill="var(--bg-lighter)" />
            {/* Foreground pie slice (Positive) */}
            <circle
              cx="60" cy="60" r="30"
              fill="none"
              ...
              strokeDashoffset={reviews.length > 0 ? (2 * Math.PI * 30) - ((positiveCount / reviews.length) * (2 * Math.PI * 30)) : 2 * Math.PI * 30}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '3rem', flex: 1 }}>
          <div style={{ borderLeft: '2px solid var(--glass-border)', paddingLeft: '1.5rem' }}>
            <div style={{ fontSize: '0.85rem', color: 'var(--text-dim)', marginBottom: '0.5rem', textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: 600 }}>Gesamt Reviews</div>
            <div style={{ fontSize: '3rem', fontWeight: 'bold', color: 'var(--text-primary)', fontFamily: '"Playfair Display", serif', lineHeight: 1 }}>{reviews.length}</div>
          </div>
          <div style={{ borderLeft: '2px solid var(--glass-border)', paddingLeft: '1.5rem' }}>
            <div style={{ fontSize: '0.85rem', color: 'var(--text-dim)', marginBottom: '0.5rem', textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: 600 }}>Positiv ({reviews.length > 0 ? `${Math.round((positiveCount / reviews.length) * 100)}%` : '0%'})</div>
            <div style={{ fontSize: '3rem', fontWeight: 'bold', color: 'var(--text-primary)', fontFamily: '"Playfair Display", serif', lineHeight: 1 }}>{positiveCount}</div>
          </div>
          <div style={{ borderLeft: '2px solid var(--glass-border)', paddingLeft: '1.5rem' }}>
            <div style={{ fontSize: '0.85rem', color: 'var(--text-dim)', marginBottom: '0.5rem', textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: 600 }}>Negativ ({reviews.length > 0 ? `${Math.round((negativeCount / reviews.length) * 100)}%` : '0%'})</div>
            <div style={{ fontSize: '3rem', fontWeight: 'bold', color: 'var(--text-primary)', fontFamily: '"Playfair Display", serif', lineHeight: 1 }}>{negativeCount}</div>
          </div>
        </div>
      </div>
        ...
        {sortedReviews.map(review => (
          <div key={review.id}>
            ...
            <div className="card-actions" style={{ marginTop: '-0.5rem', marginBottom: '1rem' }}>
              ...
              <button className="btn btn--ghost btn--sm" onClick={() => handleDelete(review)}>Löschen</button>
            </div>
          </div>
        ))}
      </div>
